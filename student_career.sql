...
SELECT * FROM StudentCareer
WHERE Interest
IN ('Data Science','HR Analytics','Artificial Intelligence');
...
SELECT COUNT(Student_Name) AS TOTAL_NUMBER_OF_STUDENT 
FROM StudentCareer;
...
SELECT SUM(Internships) AS INTERSHIP_COMPLETED
FROM StudentCareer;

# Q23. Write an SQL query to find the number of students in each department.
SELECT Department,COUNT(Student_Name) FROM StudentCareer
GROUP BY Department;

# Q24. Write an SQL query to calculate the average CGPA department-wise.
SELECT Department,AVG(CGPA) FROM StudentCareer
GROUP BY Department;
...
SELECT Gender,COUNT(Gender) FROM StudentCareer
GROUP BY Gender;

# Q27. Write an SQL query to count students based on their placement status.
SELECT Placement_Status,COUNT(Placement_Status) FROM StudentCareer
